...
def _proventos_descontos_row(row):
    etapa = str(row.get("etapa", ""))
    salario_base = float(row.get("salario_base", 0.0) or 0.0)
    dias_trabalhados = float(row.get("dias_trabalhados", 0.0) or 0.0)
    dias_desconto = float(row.get("dias_desconto", 0.0) or 0.0)
    ferias_dias = row.get("ferias_dias", "")
    decimo = float(row.get("decimo_terceiro", 0.0) or 0.0)

    proventos = []
    descontos = []

    if "Pagamento" in etapa:
        ferias_bruto = float(row.get("ferias_bruto", 0.0) or 0.0)
        ferias_salario = float(row.get("ferias_salario", 0.0) or 0.0)
        ferias_terco = float(row.get("ferias_terco", 0.0) or 0.0)
        if ferias_bruto == 0.0:
        ...
        inss_salario = float(row.get("inss_salario", 0.0) or 0.0)
        inss_ferias = float(row.get("inss_ferias", 0.0) or 0.0)
...
def gerar_pdf_pagamento(brasao_path, dados, resultado):
    ...
    _draw_label_val(c, col_left_x, left_right, y, "Conselheira", dados.get("nome", ""))
    y -= line_gap
    _draw_label_val(c, col_left_x, left_right, y, "Referencia", dados.get("referencia", ""))
    y -= line_gap
    ...
    if dados.get("competencia"):
        _draw_label_val(c, col_left_x, left_right, y, "Competencia", dados.get("competencia"))
        y -= line_gap
    _draw_label_val(
        c,
        col_left_x,
    ...
    if proventos or descontos:
        ...
        desconto_faltas = max(float(valor_original) - float(valor_ajustado), 0.0)
    ...
    c.setFont("Helvetica-Bold", 11)
    c.drawString(col_left_x, y_tot, f": {_fmt_moeda_br(resultado.get('liquido'))}")
...
if menu == "➕ Novo Pagamento":
    ...
    with col2:
        referencia = st.text_input("Ref.", value="01/2026")    
    ...
    with col1:
        valor = st.number_input("Valor bruto Salarial", min_value=3645.32, step=100.0)
    with col2:
...
# =========================
    ...
    if df.empty:
        ...
        if selecao:
            idx = selecao[1]
            row = df.loc[idx]
            proventos, descontos = _proventos_descontos_row(row)
            dados_pdf = {
                "nome": row.get("nome", ""),
                ...
                "tipo": row.get("tipo", ""),
                "etapa": row.get("etapa", ""),
                "valor": row.get("bruto", 0.0),
                "valor_original": row.get("salario_base", row.get("bruto", 0.0)),
                "valor_ajustado": row.get("salario_ajustado", row.get("bruto", 0.0)),
            ...
            resultado_pdf = {
                "bruto": row.get("bruto", 0.0),
                "base_inss": row.get("base_inss", 0.0),
                "inss": row.get("inss", 0.0),
                "base_ir": row.get("base_ir", 0.0),
                "ir_base": row.get("ir_base", 0.0),
                "redutor": row.get("redutor", 0.0),
                "irrf": row.get("irrf", 0.0),
                "liquido": row.get("liquido", 0.0)
            }
